  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Case Management</h1>
          <p className="text-slate-500 dark:text-slate-400">New York Matrimonial Division</p>
        </div>
        <Link className="bg-slate-900 dark:bg-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-slate-800 dark:hover:bg-blue-500 transition-colors"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard title="Pending Service" value={pendingServiceCount} icon={<Clock className="h-4 w-4" />} color="text-amber-600 dark:text-amber-500" />
        <StatCard title="Urgent Deadlines" value={urgentDeadlinesCount} icon={<AlertCircle className="h-4 w-4" />} color="text-red-600 dark:text-red-500" />
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
            <tr>
              <th className="px-6 py-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Client / Matter</th>
              <th className="px-6 py-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Status</th>
              <th className="px-6 py-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Grounds</th>
              <th className="px-6 py-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Next Deadline</th>
              ...
              return (
                <tr key={c.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-medium text-slate-900 dark:text-slate-100">{client?.first_name} {client?.last_name}</div>
                    <div className="text-xs text-slate-400 dark:text-slate-500 font-mono uppercase">{c.case_number || 'UNASSIGNED'}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{c.grounds}</td>
                  <td className="px-6 py-4 text-sm">
                    {nextDeadline ? (
                      <span className="text-amber-700 dark:text-amber-400 font-medium">{nextDeadline.title} ({nextDeadline.due_date})</span>
                    ) : (
                      <span className="text-slate-400 dark:text-slate-500">No pending tasks</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <Link className="text-sm font-medium text-slate-900 dark:text-blue-400 hover:underline" href={`/admin/cases/${c.id}`}>
                      View File
                    </Link>
                  </td>
                </tr>
      ...
      <div className="flex gap-4 justify-center items-center">
        {currentPage > 0 ? (
          <Link href={`?page=${currentPage - 1}`} className="text-slate-900 dark:text-slate-100 hover:underline">
            Previous
          </Link>
        ) : (
          <span className="text-slate-400 dark:text-slate-600">Previous</span>
        )}
        <span className="text-sm dark:text-slate-400">Page {currentPage + 1} of {totalPages}</span>
        {currentPage < totalPages - 1 ? (
          <Link href={`?page=${currentPage + 1}`} className="text-slate-900 dark:text-slate-100 hover:underline">
            Next
          </Link>
        ) : (
          <span className="text-slate-400 dark:text-slate-600">Next</span>
        )}
      </div>
    </div>
  );
...
function StatCard({ title, value, icon, color = "text-slate-600" }: any) {
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
      ...
      <div className="text-2xl font-bold mt-2 text-slate-900 dark:text-slate-100">{value}</div>
    </div>
  );
}
